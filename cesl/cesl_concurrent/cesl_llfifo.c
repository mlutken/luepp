...
int cesl_llfifo_empty (llfifo_t* self)
{
    return (llfifo_write_index(self) == llfifo_read_index(self));
}

char* cesl_llfifo_front (llfifo_t* self)
{
    return llfifo_get_ptr_to_index(self, llfifo_read_index(self));
}

char* cesl_llfifo_element_at (llfifo_t* self, size_t index)
{
    return llfifo_get_ptr_to_index(self, index);
}

int cesl_llfifo_full (llfifo_t* self)
{
...
int cesl_llfifo_start_push (llfifo_t* self, size_t*  write_index)
{
...
int cesl_llfifo_end_push (llfifo_t* self, size_t write_index)
{
    llfifo_write_index_set(self, write_index);
    return 1;
}

int cesl_llfifo_pop (llfifo_t* self)
{
    if (cesl_llfifo_empty (self)) {
        return 0;
    }
